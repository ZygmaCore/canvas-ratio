...
12. Review Task Dump scheduled cells only when they exist in the JSON.
13. Do not invent new project categories.
...
function buildDailyReviewTasks(
  ...
  return day.tasks
    .map((task) => {
      ...
      const firstTaskBlock = blocks.find(
        (block) =>
          block.state === "colored" &&
          blockIndexes.includes(block.index),
      );
      ...
      return {
        ...
        taskName: task.taskName,
        projectId,
        projectName: firstTaskBlock?.projectName ?? task.projectName ?? projectId,
        source,
        blockIndexes,
